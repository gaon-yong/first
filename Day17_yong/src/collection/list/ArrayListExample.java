...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		 ...
		 */
		//다형성으로 인하여 List로도 ArrayList 가능
		List<String> nick = new ArrayList<>();
		
		String str = "야옹이";
		
		//add(객체): 리스트에 객체를 추가하는 메서드, 중복이 가능하고
		//객체 내용을 보기 위해서 Arrays.toString을 쓰지 않아도 된다.
		nick.add("멍멍이");
		nick.add(str);
		...
		//add(인덱스, 객체): 특정 인덱스에 객체를 삽입하는 메서드 <-오버라이딩
		nick.add(3, "어흥이");
		System.out.println(nick);
		
		//get(인덱스): 리스트 내부의 객체를 참조하는 메서드.
//		String name = nick[3];(x) <- 이건 배열의 접근 방법 
		...
		//List는 객체만 받아주기 때문에 객체형으로 넣어야한다
		//char -> Character , int -> Integer
		score.add(65);
		
		//한번에 객체 많이 추가하기
		Collections.addAll(score, 78,45,12,564,100);
