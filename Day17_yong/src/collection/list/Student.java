...
public class Student {
	...
	public void inputStuInfo() {
		...
		while(true) {
			try {
				...
				sc.nextLine();// 이것을 넣어줘야 쓰레기가 수거되고 반복문 탈출 <- 이게 없으면 무한반복 
			}
		}
	}
	...
	public void calcTotAvgGrade() {
		...
		this.average = this.total / 3.0; //<- average가 double 이라서 3.0
		if(this.average >= 90) this.grade = "A";
		else if(this.average >=80) this.grade = "B";
		else if(this.average >=70) this.grade = "C";
		else if(this.average >=30) this.grade = "D";
		else this.grade = "F";
	}
	//ctrl+a 를 눌러서 전체 선택하고 ctrl+ i를 누르면 정리해줌
