...
public class Point {
	...
	public void inputPoints(List<Student> students) {
		...
		students.add(stu);
		System.out.printf("%s님의 성적정보가 정상적으로 입력되었습니다.\n"
				, stu.getName());

	}

	//2. 전체 학생들이 성적 정보를 출력할 메서드
	public void shwAllPoints(List<Student> students) {//학생 객체들이 들어있는 리스트가 와야죠
			...
			if(stuNum.equals(stu.getStuId())){
				...
				while(true) {
					try {
						...
						System.out.print("수정할 국어 점수를 입력해주세요: ");
						stu.setKor(sc.nextInt());
						System.out.print("수정할 영어 점수를 입력해주세요: ");
						stu.setEng(sc.nextInt());
						System.out.print("수정할 수학 점수를 입력해주세요: ");
